...
function drawRadar()
	...
	if disableGTASAhealth then
		setPlayerHudComponentVisible("health", false)
	end
	if disableGTASAarmor then
		setPlayerHudComponentVisible("armour", false)
	end
	if disableGTASAoxygen then
		setPlayerHudComponentVisible("breath", false)
	end

	if (not isPlayerMapVisible()) then
		local mW, mH = dxGetMaterialSize(rt)
		if (alwaysRenderMap and getElementInterior(localPlayer) == 0) then
			dxSetRenderTarget(rt, true)
			local x, y = getElementPosition(localPlayer)
			local X, Y = mW / 2 - (x / (6000 / worldW)), mH / 2 + (y / (6000 / worldH))
			local camX, camY, camZ = getElementRotation(getCamera())
			dxDrawImage(X - worldW / 2, mH / 5 + (Y - worldH / 2), worldW, worldH, "img/radar_mapV2.png", camZ, (x / (6000 / worldW)), -(y / (6000 / worldH)), tocolor(255, 255, 255, 255))
			dxSetRenderTarget()
		end
		...
		local _, _, camZ = getElementRotation(getCamera())
		dxDrawImage(cX - (blip * 1.5) * sW / 2 + 10, cY - (blip * 1.5) * sH / 2 -12, (blip * 1.5) * sW, (blip * 1.5) * sH, "img/player.png", camZ - rz, 0, 0)
